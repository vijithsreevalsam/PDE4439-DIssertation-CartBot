...
if [ "$*" == "" ]
    then
        ...
        echo
        echo "Example: $ bash install_cartBot.bash 2wd a1"
        echo "Example: $ bash install_cartBot.bash 2wd a1 realsense"
        echo "Example: $ bash install_cartBot.bash 2wd - realsense"
        echo "Example: $ bash install_cartBot.bash 2wd"
...
if [[ "$BASE" != "ci" && "$LASER_SENSOR" != "" && "$LASER_SENSOR" != "-" ]] && !(printf '%s\n' "${LASER_SENSOR_ARRAY[@]}" | grep -xq $LASER_SENSOR)
    then
        echo "Invalid cartBot laser sensor: $LASER_SENSOR"
        echo
        echo "Valid Options:"
...
if [[ "$BASE" != "ci" && "$DEPTH_SENSOR" != "" ]] && !(printf '%s\n' "${DEPTH_SENSOR_ARRAY[@]}" | grep -xq $DEPTH_SENSOR)
    then
        echo "Invalid cartBot depth sensor: $DEPTH_SENSOR"
        echo
        echo "Valid Options:"
...
if [[ "$BASE" != "ci" ]]
    then
        echo
        echo "You are installing cartBot on your robot computer."
        echo
        echo "===========SUMMARY============"
        ...
        echo "Create a cartBot_ws on your $HOME directory."
        echo "Install cartBot ROS2 dependencies."
...
#### 2.1 Download cartBot:
cd $WORKSPACE
git clone -b $ROS_DISTRO https://github.com/linorobot/cartBot src/cartBot

#### 2.2 Ignore Gazebo Packages on robot computer (optional)
cd $WORKSPACE/src/cartBot/cartBot_gazebo
touch COLCON_IGNORE

#### 2.3 Install cartBot package:
cd $WORKSPACE
...
if [[ "$BASE" != "ci" ]]
    then
        ### 1. Robot Type
        echo "export cartBot_BASE=$BASE" >> ~/.bashrc
        ### 2. Sensors
        if [[ "$LASER_SENSOR" != "-" ||  "$LASER_SENSOR" != "" ]]
            then
                echo "export cartBot_LASER_SENSOR=$LASER_SENSOR" >> ~/.bashrc
        fi

        if [[ "$DEPTH_SENSOR" != "-" ||  "$DEPTH_SENSOR" != "" ]]
            then
            echo "export cartBot_DEPTH_SENSOR=$DEPTH_SENSOR" >> ~/.bashrc
        fi
        echo
        echo "Do you want to add sourcing of cartBot_ws on your ~/.bashrc?"
        echo -n "Yes [y] or No [n]: " 
        read reply
        if [[ "$reply" == "y" || "$reply" == "Y" ]]
            then
                echo "source \$HOME/cartBot_ws/install/setup.bash" >> ~/.bashrc
        else
            echo
            echo "Remember to run $ source ~/cartBot_ws/install/setup.bash every time you open a terminal."
        fi
fi
